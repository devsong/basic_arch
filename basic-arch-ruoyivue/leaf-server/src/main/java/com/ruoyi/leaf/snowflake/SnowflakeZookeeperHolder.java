package com.ruoyi.leaf.snowflake;
...
import com.ruoyi.leaf.snowflake.exception.CheckLastTimeException;
...
public class SnowflakeZookeeperHolder {
    ...
    public boolean init() {
        try {
            ...
            curator.start();
            if (curator.checkExists().forPath(PATH_FOREVER) == null) {
                ...
                if (workerid != null) {
                    ...
                    doService(curator);
                    updateLocalWorkerID(workerID);
                    LOGGER.info("[Old NODE]find forever node have this endpoint ip-{} port-{} workid-{} childnode and start SUCCESS", ip,
                            port, workerID);
                } else {
    ...
    private CuratorFramework createWithOptions(String connectionString, RetryPolicy retryPolicy, int connectionTimeoutMs,
