...
public interface SerialPartitionMapper extends Mapper<SerialPartitionPo> {
    String GET_SERIAL_PARTITION_LOCK = "SELECT `name`, version, part, stat, min, max,used,tscr FROM serialpart AS P WHERE P.`name` = #{name} AND P.version = #{ver} AND P.part = #{part} FOR UPDATE;";
