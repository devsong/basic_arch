...
JAVA_OPTS="-server -Xms128m -Xmx128m -XX:NewSize=32m -XX:MaxNewSize=64m -Xss256k -Ddubbo.application.logger=slf4j"
...
function start() {
    ...
    if [ $running -gt 0 ];then
 	...
 	nohup java $JAVA_OPTS -cp $jar_home/lib/*:$jar_home/conf:$jar_home/$module $main >> ${logfile} 2>&1 & pid=$!
