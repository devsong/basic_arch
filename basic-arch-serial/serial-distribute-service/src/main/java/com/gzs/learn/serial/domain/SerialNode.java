...
public class SerialNode implements Serializable {
     ...
     * @param notify
     */
    public SerialNode(String key, int version, long tsup, long position, long max, int step, long limit,
            LinkedBlockingQueue<GroupListenerNode> notify) {
        super();
        this.key = key;
    ...
    public long getSerial(int length) {
        ...
        if (length < 0 || length > this.step) {
            throw new IllegalArgumentException(String.format("Serial length support [1~%d].", this.step));
        }

        try {
            if (this.lock.tryLock() || this.lock.tryLock(10, TimeUnit.MILLISECONDS)) {
                GroupListenerNode groupListenerNode = new GroupListenerNode(this.key, this.version, this.tsup, NodeType.RECHARGE);
                if (position + length <= max) {
                    ret = position;
                    position += length;
                } else if (this.notifySwitch) {
                    this.specialSwitch = true;
                    this.notify.offer(groupListenerNode);
                } else {
                    this.release++;
                }
